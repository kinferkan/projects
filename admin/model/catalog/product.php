...
class ModelCatalogProduct extends Model {
    ...
    public function addProduct($data) {
        $this->db->closeAutocommit();
        
        $result = $this->db->query("INSERT INTO " . DB_PREFIX . "product SET status = '" . (int) $data['status'] ."', sort_order = '" . (int) $data['sort_order'] . "', date_added = NOW()");

        $product_id = $this->db->getLastId();

        if (isset($data['image'])) {
            $result = $this->db->query("UPDATE " . DB_PREFIX . "product SET image = '" . $this->db->escape($data['image']) . "' WHERE product_id = '" . (int) $product_id . "'");
        }

        foreach ($data['product_description'] as $language_id => $value) {
           $result = $this->db->query("INSERT INTO " . DB_PREFIX . "product_description SET product_id = '" . (int) $product_id . "', language_id = '" . (int) $language_id . "', name = '" . $this->db->escape($value['name']) . "', description = '".$this->db->escape($value['description'])."', reason = '".$this->db->escape($value['reason'])."'");
        }

        $user_id = $this->user->isLogged();
        if ($user_id) {
           $result = $this->db->query("INSERT INTO ".DB_PREFIX."product_user SET user_id = '".(int)$user_id."', product_id = '".(int)$product_id."', date_added = NOW()");
        }

        if (isset($data['product_image'])) {
            foreach ($data['product_image'] as $image) {
                $result = $this->query("INSERT INTO " . DB_PREFIX . "product_image SET product_id = '" . (int) $product_id . "', image = '" . $this->db->escape($image) . "'");
            }
        }

        if (isset($data['product_category'])) {
            foreach ($data['product_category'] as $category_id) {
                $result = $this->db->query("INSERT INTO " . DB_PREFIX . "product_to_category SET product_id = '" . (int) $product_id . "', category_id = '" . (int) $category_id . "'");
            }
        }
    ...
    public function editProduct($product_id, $data) {
        $name = '';
        $this->db->query("UPDATE " . DB_PREFIX . "product SET date_available = '" . $this->db->escape($data['date_available']) . "', manufacturer_id = '" . (int) $data['manufacturer_id'] . "', price = '" . (float) $data['price'] . "', points = '" . (int) $data['points'] ."', status = '" . (int) $data['status'] ."', sort_order = '" . (int) $data['sort_order'] . "', date_modified = NOW() WHERE product_id = '" . (int) $product_id . "'");
